...
knex.schema.hasTable('rirs_kobo').then(function (exists) {
  if (!exists) {
    return knex.schema.createTable('rirs_kobo', function (t) {
...
var options = {
  method: 'GET',
  url: 'http://me.wfp.org.pk:8001/api/v1/data/16',
  headers: {
    'Content-Type': 'application/json',
    Authorization: 'Token 6001d69ff8eeeca1a2f8853a6fb23eafe53ae114'
  }
};
// adding unique data to mysql
// async.waterfall([
//   function(callback){
...
//   result.forEach(el=>{
//     var x = {};
//     x.body = JSON.stringify(el);
//     x.uuid = el._uuid;
//     // x.test = el;
//     x.created_at = knex.fn.now()
//     knex('rirs_kobo_email')
//       .insert(x)
//       .whereNot({uuid: x.uuid})
//       .then(imran=>{
//         console.log(imran)
//       })
//       .catch(err=>{
...
knex('rirs_kobo')
  ...
  .then(single => {
    ...
    transporter.sendMail(mailOpts(JSON.parse(single[0].body)), (error, info) => {
      if (error) {
